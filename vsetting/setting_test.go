...
import (
	...
	"github.com/imajinyun/go-knifer/vsetting"
)

func TestParseSettingFacade(t *testing.T) {
	s, err := vsetting.Parse("name=gokit\ncount=42\nenabled=true\n[server]\nhost=127.0.0.1\nport=8080")
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadAndParseYAMLFacade(t *testing.T) {
	...
	s, err = vsetting.ParseYAML("app: gokit\nserver:\n  port: 8080\n  debug: true")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewAndParseBytesFacade(t *testing.T) {
	...
	parsed, err := vsetting.ParseBytes([]byte("x: 1"))
	if err != nil {
		t.Fatal(err)
	}
