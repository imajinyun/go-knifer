...
| --- | --- | --- |
| TOTP and HOTP | Callers currently compose one-time password workflows outside `knifer-go`. | Add RFC-compatible helpers only with deterministic clock/counter injection, explicit issuer/account formatting, and constant-time verification. |
