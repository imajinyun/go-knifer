...
| `vresty` | `github.com/imajinyun/go-knifer/vresty` | Resty v3 based HTTP facade: chainable requests, JSON/form/multipart bodies, isolated/global-config request construction, create/get/post `WithOptions` helpers, downloads, and lightweight response helpers. |
...
| Cron / DFA / ID / identity | `vcron.WithDefaultSchedulerOptions`, `vcron.WithIDRandomReader`, `vcron.CronScheduleWithOptions`, `vdfa.WithMatcherWords`, `vdfa.ContainsWithOptions`, `vid.NewIsolatedSnowflake`, `vid.CreateSnowflakeWithOptions`, `vid.WithSnowflakeCache`, `vid.WithFallbackRandomSource`, `vident.BirthDateWithOptions` |
